				...
				const uint16_t output_ch, q15_t *runtime_buf, q7_t pad_value)
{
	...
	for (c = 0; c < input_ch; c++){        
		...
		depthwise_kernel5x5_stride2_inplace_kernel_CHW(output_y, output_x, bias++, biasR++, ksrc, scales++, inplace_out, output_offset,output_activation_min, output_activation_max,cols_8b_start, input_x, input_ch);
		ksrc += 25;
		input++;
    }
