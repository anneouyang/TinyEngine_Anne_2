...
tinyengine_status convolution_s8_kernel7x7_stride3_pad3x3_input1(q7_t *input, const uint16_t input_x, const uint16_t input_y,
    ...
    for (i_out_y = 0; i_out_y < output_y; i_out_y++) {
		for (i_out_x = 0; i_out_x < output_x; i_out_x++) {
            ...
            if (base_idx_y >= 0) {
                ...
                cnt = base_idx_y * -1;
                while (cnt--){
                    *dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;          
                }
                if (base_idx_x >= 0) {
			...
			if (two_column_buf == runtime_buf + 2 * 49) {
