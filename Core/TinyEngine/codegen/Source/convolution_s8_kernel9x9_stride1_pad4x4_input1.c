...
/* ----------------------------------------------------------------------
 * Project:      TinyEngine
 * Description:  for normal convolution 9x9
 * Target ISA:  ARMv7E-M
...
tinyengine_status convolution_s8_kernel9x9_stride1_pad4x4_input1(q7_t *input, const uint16_t input_x, const uint16_t input_y,
				...
				const uint16_t output_ch, q15_t *runtime_buf, q7_t pad_value)
{
    ...
    for (i_out_y = 0; i_out_y < output_y; i_out_y++) {
		for (i_out_x = 0; i_out_x < output_x; i_out_x++) {
			...
			const int16_t base_idx_y = (i_out_y  * 1) - 4;
			const int16_t base_idx_x = (i_out_x  * 1) - 4;
			...
			q15_t *dst0 = &col_buffer[9 * input_ch * 0];
			q15_t *dst1 = &col_buffer[9 * input_ch * 1];
			q15_t *dst2 = &col_buffer[9 * input_ch * 2];
			q15_t *dst3 = &col_buffer[9 * input_ch * 3];
			q15_t *dst4 = &col_buffer[9 * input_ch * 4];
			q15_t *dst5 = &col_buffer[9 * input_ch * 5];
			q15_t *dst6 = &col_buffer[9 * input_ch * 6];
			q15_t *dst7 = &col_buffer[9 * input_ch * 7];
			q15_t *dst8 = &col_buffer[9 * input_ch * 8];
            int input_row_offset = input_ch * input_x;
            //load 
            if (base_idx_y >= 0) {
                if (base_idx_x >= 0) {
                    //9 elements, kernel_w * input_ch
                    ...
                    src7= src6 + input_row_offset;
                    src8= src7 + input_row_offset;
                    //9 = 4 * 2 + 1
					...
					*dst6++ = *src6++ + input_offset;
                    q7_q15_offset_ele(src7, dst7)
                    q7_q15_offset_ele(src7, dst7)
					*dst7++ = *src7++ + input_offset;
                    q7_q15_offset_ele(src8, dst8)
                    q7_q15_offset_ele(src8, dst8)
					*dst8++ = *src8++ + input_offset;
                } else {
                ...
                cnt = base_idx_y * -1;
                while (cnt--){
                    *dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;*dst0++ = pad_out;          
                }
                if (base_idx_x >= 0) {
                    for (i = 0; i < 9 + base_idx_y; i++){//rows
            ...
            if (base_idx_x > input_x - 9){
				cnt = base_idx_x - (input_x - 9);
                dst0 = &col_buffer[input_ch * (9-cnt)];
                dst1 = dst0 + 9* input_ch;
                dst2 = dst1 + 9* input_ch;
                dst3 = dst2 + 9* input_ch;
                dst4 = dst3 + 9* input_ch;
                dst5 = dst4 + 9* input_ch;
                dst6 = dst5 + 9* input_ch;
                dst7 = dst6 + 9* input_ch;
                dst8 = dst7 + 9* input_ch;
                while(cnt--){
                    *dst0++ = pad_out;
                    *dst1++ = pad_out;
                    *dst2++ = pad_out;
            ...
            if (base_idx_y > input_y - 9){
				cnt = base_idx_y - (input_y - 9);
            	dst0 = &col_buffer[9 * input_ch * (9-cnt)];
            	while(cnt--){
                    *dst0++ = pad_out;
                    *dst0++ = pad_out;
                    *dst0++ = pad_out;
			...
			if (two_column_buf == runtime_buf + 2 * 81) {
	...
	if (two_column_buf != runtime_buf) {
		...
		for (i = 0; i < output_ch; i++) {
			...
			uint16_t col_count = (input_ch * 81) >> 2;
