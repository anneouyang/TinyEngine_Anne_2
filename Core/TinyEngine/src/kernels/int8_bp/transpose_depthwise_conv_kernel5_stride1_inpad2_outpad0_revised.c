/* ----------------------------------------------------------------------
 * Name: transpose_depthwise_conv_kernel5_stride1_inpad2_outpad0_revised.c
  ...
  for (c = 0; c < input_depth; c++) {
    ...
    for (i = 0; i < output_height; i++) {
      for (j = 0; j < output_width - 1; j+=2) {
        ...
        int32_t sum_0 = 0;
        int32_t sum_1 = 0;
        transpose_depthwise_mac_kernel5_2row_uniweight(&sum_0, &sum_1, two_column_buffer, ksrc_transposed, input_width, STRIDE, IN_PAD, OUT_PAD);
      ...
      if (output_width & 0x1) {
        ...
        // We assume bias_data as zeros.
        int32_t sum_0 = 0;
        transpose_depthwise_mac_kernel5_1row_uniweight(&sum_0, two_column_buffer, ksrc_transposed, input_width, STRIDE, IN_PAD, OUT_PAD);
