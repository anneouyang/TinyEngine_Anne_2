...
int Offset3(const uint16_t dims_data1, const uint16_t dims_data2, const uint16_t dims_data3, int i0, int i1, int i2, int i3) {
  /*
...
int gOffset(const uint16_t dims_data1, const uint16_t dims_data2, const uint16_t dims_data3, const uint16_t dims_data4, int i0, int i1, int i2, int i3, int i4) {
  /*
  TFLITE_DCHECK_EQ(shape.DimensionsCount(), 4);
  const int* dims_data = reinterpret_cast<const int*>(shape.DimsDataUpTo5D());
  TFLITE_DCHECK((dims_data[0] == 0 && i0 == 0) ||
  ...
  TFLITE_DCHECK((dims_data[3] == 0 && i3 == 0) ||
                (i3 >= 0 && i3 < dims_data[3]));
  */
  int offset = (((i0 * dims_data1 + i1) * dims_data2 + i2) * dims_data3 + i3) * dims_data4 + i4;
  return offset;
}


float _ActivationFunctionWithMinMax(float x, float output_activation_min,
...
int32_t _MultiplyByQuantizedMultiplier(int32_t x, int32_t quantized_multiplier, int shift) {
      ...
      for (int out_y = 0; out_y < output_height; ++out_y) {
        ...
        const int in_x_origin = (out_x * stride_width) - pad_width;
          for (int out_channel = 0; out_channel < output_gdepth; ++out_channel) {
            ...
            output_data[Offset3(output_height, output_width, _output_depth, batch, out_y, out_x, out_channel + g * output_gdepth)] =
                _ActivationFunctionWithMinMax(total + bias_value,
...
tinyengine_status_fp group_conv_int8_PerChannel(const Conv_Params params, int group,
                 ...
                 int8_t* output_data, const uint16_t output_height, const uint16_t output_width, const uint16_t _output_depth,
  ...
  int32_t input_value, filter_value;
      ...
      for (int out_y = 0; out_y < output_height; ++out_y) {
        ...
        for (int out_x = 0; out_x < output_width; ++out_x) {
          ...
          for (int out_channel = 0; out_channel < output_gdepth; ++out_channel) {
            ...
            total = _MultiplyByQuantizedMultiplier(
                total, output_multiplier[out_channel], output_shift[out_channel]);
            total += output_offset;
