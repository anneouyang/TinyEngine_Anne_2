/* ----------------------------------------------------------------------
 * Name: transpose_conv_fp_kernel3_stride2_inpad1_outpad1.c
...
tinyengine_status_fp transpose_conv_fp_kernel3_stride2_inpad1_outpad1(const float* input_data, const uint16_t input_height, const uint16_t input_width, const uint16_t input_depth, 
  ...
  uint16_t output_height = final_output_height + 2;
  uint16_t output_width = final_output_width + 2;
  int i, j, c;
    ...
    for (int i_in_x = 0; i_in_x < input_width - 1; i_in_x+=2) {
      ...
      while (row_count--) {
        ...
        while (input_depth_div4--) {
          ...
          out2 = out2_start;
          out3 = out3_start;
          transpose_mac_3row_3col_2input_2filter_2stride_fp(out, out2, out3, output_depth, input_0, input_1, filter_0, filter_1);
          input_0++; input_1++;
      ...
      if (output_depth & 0x1) {
        ...
        while (input_depth_div4--) {
          ...
          out2 = out2_start;
          out3 = out3_start;
          transpose_mac_3row_3col_2input_1filter_2stride_fp(out, out2, out3, output_depth, input_0, input_1, filter_0);
          input_0++; input_1++;
          filter_0 += DIM_KER_X * DIM_KER_Y;
  ...
  for (c = 0; c < output_depth; c++) {
    for (i = 0; i < final_output_height; i++) {
      for (j = 0; j < final_output_width; j++) {
        output_data[(i * final_output_width + j) * output_depth + c] = output_data[((i+1) * output_width + (j+1)) * output_depth + c];
      }
    }
  }
} 
