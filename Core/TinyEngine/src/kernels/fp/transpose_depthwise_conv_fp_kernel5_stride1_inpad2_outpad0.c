/* ----------------------------------------------------------------------
 * Name: transpose_depthwise_conv_fp_kernel5_stride1_inpad2_outpad0.c
  ...
  for (c = 0; c < input_depth; c++) {
    ...
    for (i = 0; i < input_height; i++) {
      for (j = 0; j < input_width - 1; j+=2) {
        ...
        transpose_mac_5row_5col_2input_1filter_1stride_fp(out, out2, out3, out4, out5, output_depth, input_0, input_1, ksrc);
