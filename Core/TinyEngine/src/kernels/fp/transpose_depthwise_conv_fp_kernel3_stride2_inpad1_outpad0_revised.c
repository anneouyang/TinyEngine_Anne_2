/* ----------------------------------------------------------------------
 * Name: transpose_depthwise_conv_fp_kernel3_stride2_inpad1_outpad0_revised.c
...
tinyengine_status_fp transpose_depthwise_conv_fp_kernel3_stride2_inpad1_outpad0_revised_IOHW_int8w_partialCH(float* input_data, 
  ...
  for (c = 0; c < input_depth; c++) {
    two_column_buffer = im2col_data + input_width * 2 + 1;
    src = input_data;
    ...
    for (i = 0; i < input_height; i++) {
      ...
      two_column_buffer += input_width * 2 + 1;
    }
