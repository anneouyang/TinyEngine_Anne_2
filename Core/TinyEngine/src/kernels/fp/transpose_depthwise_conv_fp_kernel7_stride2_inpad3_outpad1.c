/* ----------------------------------------------------------------------
 * Name: transpose_depthwise_conv_fp_kernel7_stride2_inpad3_outpad1.c
  ...
  for (c = 0; c < input_depth; c++) {
    ...
    for (i = 0; i < input_height; i++) {
      for (j = 0; j < input_width - 1; j+=2) {
        ...
        transpose_mac_7row_7col_2input_1filter_2stride_fp(out, out2, out3, out4, out5, out6, out7, output_depth, input_0, input_1, ksrc);
