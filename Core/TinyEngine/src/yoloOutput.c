...
int postprocess_single(float *data, int w, int h, float stride, int num_classes,
		const int anchors[3][2], const int num_anchors, det_box **box,
...
void data_requantize(int8_t *input_data, float *output_data, int size,
		int8_t y_zero, float y_scale) {
	int i;
	for (i = 0; i < size; i++) {
...
int omni_nms(det_box *candidates, det_box *results, float NMS_threshold, int candicate_num) {
	...
	while (1) {
		...
		for (int i = 0; i < candicate_num; i++) {
			if (!suppress[i]) {
				...
				float area_i = (candidates[i].x1 - candidates[i].x0)
...
det_box** postprocessing(signed char *input_data[3], signed char y_zero[3], float y_scale[3],
	...
	for (i = 0; i < 3; i++) {
		...
		postprocess_single(out, w / strides[i], h / strides[i], strides[i],
				num_classes, anchors[i], 3, candidates, VALID_THRESHOLD);
	}
	for (i = 0; i < num_classes; i++) {
		box_ret[i] = omni_nms(candidates[i], ret_box[i], NMS_threshold, boxcnt[i]);
	}
	return NULL;
}
...
det_box** postprocessing_fp(float *input_data[3], signed char y_zero[3], float y_scale[3],
	...
	for (i = 0; i < 3; i++) {
		...
		int size = (w / strides[i]) * (h / strides[i]) * output_c;
		for (j = 0; j < size; j++) {
			input_data[i][j] -= (float)y_zero[i];
			input_data[i][j] *= y_scale[i];
		}
		postprocess_single(input_data[i], w / strides[i], h / strides[i], strides[i],
				num_classes, anchors[i], 3, candidates, VALID_THRESHOLD);
	}
	for (i = 0; i < num_classes; i++) {
		box_ret[i] = omni_nms(candidates[i], ret_box[i], NMS_threshold, boxcnt[i]);
